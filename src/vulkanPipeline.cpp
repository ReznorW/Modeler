...
std::array<VkVertexInputAttributeDescription, 4> Vertex::getAttributeDescriptions() {
    ...
    attributeDescriptions[3].offset = offsetof(Vertex, isSelected);
...
VulkanPipeline::VulkanPipeline(VulkanDevice& device, VkRenderPass renderPass, VkDescriptorSetLayout descriptorSetLayout, const PipelineConfig& config, const std::string& shaderPath)
    ...
    VkShaderModule fragShaderModule = createShaderModule(deviceRef.getLogicalDevice(), fragShaderCode);
    ...
    if (vkCreateGraphicsPipelines(deviceRef.getLogicalDevice(), VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &graphicsPipeline) != VK_SUCCESS) {
        throw std::runtime_error("Failed to create graphics pipeline.");
    }

    // Cleanup shader modules
    vkDestroyShaderModule(deviceRef.getLogicalDevice(), fragShaderModule, nullptr);
    vkDestroyShaderModule(deviceRef.getLogicalDevice(), vertShaderModule, nullptr);
}

VulkanPipeline::~VulkanPipeline() {
    vkDestroyPipeline(deviceRef.getLogicalDevice(), graphicsPipeline, nullptr);
    vkDestroyPipelineLayout(deviceRef.getLogicalDevice(), pipelineLayout, nullptr);
}
